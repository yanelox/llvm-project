...
static const MCPhysReg ArgGPRs[] = {MyArch::X9, MyArch::X10, MyArch::X11, MyArch::X12};

void MyArchTargetLowering::ReplaceNodeResults(SDNode *N,
...
MyArchTargetLowering::MyArchTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(MyArch::X1);
...
#include "MyArchGenCallingConv.inc"
...
SDValue MyArchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_MyArch);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, MyArch::X1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(MyArchISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_MyArch);
...
/// MyArch formal arguments implementation
SDValue MyArchTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_MyArch);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &MyArch::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    MyArchFunctionInfo *UFI = MF.getInfo<MyArchFunctionInfo>();
...
bool MyArchTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_MyArch))
    return false;
...
SDValue
MyArchTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
  ...
  const MyArchSubtarget &STI = MF.getSubtarget<MyArchSubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_MyArch);
  ...
  return DAG.getNode(MyArchISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue MyArchTargetLowering::PerformDAGCombine(SDNode *N,
...
bool MyArchTargetLowering::isLegalAddressingMode(const DataLayout &DL,
